...
public class Rol extends com.F64.Codepoint {
	...
	public void generate(Builder b)
	{
		if (dest == -1) {
			if (cnt == -1) {
				b.add(RegOp3.ROL, Register.T.ordinal(), Register.S.ordinal(), Register.T.ordinal());
				b.add(ISA.NIP);
			}
			else {
				b.add(RegOp3.ROLI, Register.T.ordinal(), Register.T.ordinal(), cnt);
			}
		}
		else if (dest == src1) {
			if (src2 >= 0) {
				b.add(RegOp2.ROL, dest, src2);
			}
			else {
				b.add(RegOp2.ROLI, dest, cnt);
			}
		}
		else if (src2 >= 0) {
			b.add(RegOp3.ROL, dest, src1, src2);
		}
		else {
			b.add(RegOp3.ROLI, dest, src1, cnt);
		}
	}
