...
//				break;
	...
	public void generate(Builder b)
	{
		if (dest == -1) {
			if (cnt == -1) {
				b.add(RegOp3.RCR, Register.T.ordinal(), Register.S.ordinal(), Register.T.ordinal());
				b.add(ISA.NIP);
			}
			else {
				b.add(RegOp3.RCRI, Register.T.ordinal(), Register.T.ordinal(), cnt);
			}
		}
		else if (dest == src1) {
			if (src2 >= 0) {
				b.add(RegOp2.RCR, dest, src2);
			}
			else {
				b.add(RegOp2.RCRI, dest, cnt);
			}
		}
		else if (src2 >= 0) {
			b.add(RegOp3.RCR, dest, src1, src2);
		}
		else {
			b.add(RegOp3.RCRI, dest, src1, cnt);
		}
	}
